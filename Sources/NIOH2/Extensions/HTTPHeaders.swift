...
extension HTTPHeaders {
    ...
    ///             ASCII string. For future-proofing with HTTP/2 lowercase header names
    ...
    ///
    /// - parameters:
    ///     - name: The name of the header field to remove from the block.
    public mutating func remove(name: HTTPHeaderName) {
        remove(name: name.lowercased)
    }
    
    /// Retrieve all of the values for a given header field from the block.
    ///
    /// This method uses case-insensitive comparison for the header field name. It
    /// does not return a maximally-decomposed list of the header fields, but instead
    /// returns them in their original representation: that means that a comman-separated
    /// header field list may contain more than one entry, some of which contain commas
    /// and some do not. If you wnat a representation of the header fields suitable for
    /// performing computation on, consider `getCanonicalForm`.
    ///
    /// - parameters:
    ///     - name: The header field name whose values are to be retrieved.
    ...
    /// - parameters:
    ///     - name: The header field name to check.
    public func contains(name: HTTPHeaderName) -> Bool {
        return contains(name: name.lowercased)
    }
